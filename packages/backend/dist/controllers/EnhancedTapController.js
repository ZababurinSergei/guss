...
import { EnhancedTapService } from '../services/EnhancedTapService.js';
import { RoundCheckService } from '../services/RoundCheckService.js';
export class EnhancedTapController {
    static async tap(request, reply) {
        try {
    ...
    static async getTapStatus(request, reply) {
        try {
            ...
            if (statusResult.status === 'completed') {
                const resultJson = await QueueService.redis.get(`completed:${taskId}`);
                if (resultJson) {
                    additionalData = { result: JSON.parse(resultJson) };
                }
            }
            else if (statusResult.status === 'failed') {
                const errorMessage = await QueueService.redis.get(`failed:${taskId}`);
                if (errorMessage) {
    ...
    static async getTapStatistics(request, reply) {
        try {
            const userId = request.user.id;
            const useQueue = process.env.USE_TAP_QUEUE === 'true';
            const stats = {
    ...
    static async batchTap(request, reply) {
        try {
            const { roundId, count = 1 } = request.body;
            const userId = request.user.id;
            if (count < 1 || count > 10) {
